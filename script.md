...
Miners, which consist of a combination of hardware and software, are constantly trying to calculate and prove that the hashes they have derived both represent the most work done and all the most recent valid transactions. 
...
The ability of miners to calculate the required hashes and hence solve the next block, is directly correlated to the number of hashes they can calculate per second (H/s). This is also known as the miners hashrate.

The Monero network aims to add a new block onto the chain approximately every 120 seconds. This is equal to around 720 blocks per day. To achieve this rate a balance is required to prevent miners from solving blocks too quickly or too slowly. This balance is called the network difficulty. The network difficulty is approximately calculated by taking the current network hash rate and multiplying it by the block target. An example follows; 2,5Gh/s × 120s = 300GH. 300G is hence the target number of hashes required to mint a new block. As the hashing power of the network increases so does the difficulty.
...
Just to start, the attacker would need more than 32000 of these CPUs costing over €208m. This is obviously quite a simple and brutal estimation, but it illustrates the point well enough.
